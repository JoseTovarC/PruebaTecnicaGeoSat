...
public class UnitTest {
    ...
    public void checkIndex() {
        ...
        final ResourceController controller = new ResourceController(formFactory, repository, ec);
    ...
    public void checkTemplate() {
        ...
        assertThat(contentAsString(html)).contains("Add Resource");
    }
    ...
    public void checkAddResource() {
        ...
        resource.id = 1L;
        resource.name = "Steve";
        when(repository.add(any())).thenReturn(supplyAsync(() -> resource));

        // Set up the request builder to reflect input
        Http.Request request = Helpers.fakeRequest("POST", "/").bodyJson(Json.toJson(resource)).build().withTransientLang(Lang.forCode("es"));
        ...
        CompletionStage<Result> stage = controller.addResource(request);
